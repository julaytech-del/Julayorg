...
export default function ProjectDetail() {
  ...
  if (loading || !project) return <LoadingSpinner fullPage message="Loading project..." />;
  ...
  const handleReplan = async () => {
    const reason = prompt('Reason for replanning?', 'Schedule adjustment needed');
    if (!reason) return;
    await dispatch(replanProject({ projectId: id, reason }));
    dispatch(showSnackbar({ message: 'Project replanned by AI!' }));
    dispatch(fetchProject(id));
  };
    ...
    <Box>
        ...
        <CardContent sx={{ p: 3 }}>
          <Box sx={{ display: 'flex', gap: 3, flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between' }}>
            <Box sx={{ flex: 1, minWidth: 200 }}>
              <Box sx={{ display: 'flex', gap: 1, mb: 1, flexWrap: 'wrap' }}>
                ...
                {project.aiGenerated && <Chip size="small" icon={<AutoAwesome sx={{ fontSize: '12px !important' }} />} label="AI Generated" sx={{ bgcolor: '#EEF2FF', color: '#6366F1', fontWeight: 600 }} />}
              </Box>
              ...
              <Box sx={{ display: 'flex', gap: 3, mt: 2, flexWrap: 'wrap' }}>
                {project.startDate && <Box><Typography variant="caption" color="text.secondary">Start</Typography><Typography variant="body2" fontWeight={500}>{format(new Date(project.startDate), 'MMM dd, yyyy')}</Typography></Box>}
                {project.endDate && <Box><Typography variant="caption" color="text.secondary">Due</Typography><Typography variant="body2" fontWeight={500}>{format(new Date(project.endDate), 'MMM dd, yyyy')}</Typography></Box>}
                <Box><Typography variant="caption" color="text.secondary">Tasks</Typography><Typography variant="body2" fontWeight={500}>{doneTasks}/{tasks.length}</Typography></Box>
              </Box>
            </Box>
            <Box sx={{ display: 'flex', gap: 1, flexWrap: 'wrap' }}>
              <Button size="small" variant="outlined" startIcon={<ViewKanban />} onClick={() => navigate(`/projects/${id}/kanban`)}>Kanban</Button>
              <Button size="small" variant="outlined" startIcon={<Timeline />} onClick={() => navigate(`/projects/${id}/timeline`)}>Timeline</Button>
              <Button size="small" variant="outlined" startIcon={<PsychologyAlt />} onClick={handleStandup} disabled={aiLoading}>Standup</Button>
              <Button size="small" variant="outlined" startIcon={<AutoAwesome />} onClick={handlePerformance} disabled={aiLoading}>Analyze</Button>
              <Button size="small" variant="outlined" startIcon={<Refresh />} onClick={handleReplan} color="warning">Re-Plan</Button>
            </Box>
          </Box>
          ...
          <Box sx={{ mt: 2, display: 'flex', alignItems: 'center', gap: 1 }}>
            <AvatarGroup max={6} sx={{ '& .MuiAvatar-root': { width: 30, height: 30, fontSize: '0.75rem' } }}>
              {(project.team || []).map(t => <Avatar key={t.user?._id} sx={{ bgcolor: 'primary.main' }}>{t.user?.name?.[0]}</Avatar>)}
            </AvatarGroup>
            <Typography variant="caption" color="text.secondary">{project.team?.length || 0} team members</Typography>
          </Box>
        </CardContent>
      </Card>
      ...
      {tab === 0 && (
        <Grid container spacing={2.5}>
          <Grid item xs={12} sm={6} lg={3}>
            <Card><CardContent><Typography variant="caption" color="text.secondary">Total Tasks</Typography><Typography variant="h4" fontWeight={700}>{tasks.length}</Typography></CardContent></Card>
          </Grid>
          <Grid item xs={12} sm={6} lg={3}>
            <Card><CardContent><Typography variant="caption" color="text.secondary">Completed</Typography><Typography variant="h4" fontWeight={700} color="success.main">{doneTasks}</Typography></CardContent></Card>
          </Grid>
          <Grid item xs={12} sm={6} lg={3}>
            <Card><CardContent><Typography variant="caption" color="text.secondary">In Progress</Typography><Typography variant="h4" fontWeight={700} color="info.main">{tasks.filter(t => t.status === 'in_progress').length}</Typography></CardContent></Card>
          </Grid>
          <Grid item xs={12} sm={6} lg={3}>
            <Card><CardContent><Typography variant="caption" color="text.secondary">Blocked</Typography><Typography variant="h4" fontWeight={700} color="error.main">{tasks.filter(t => t.status === 'blocked').length}</Typography></CardContent></Card>
          </Grid>
          <Grid item xs={12}>
            <Card><CardContent>
              <Typography variant="h6" fontWeight={600} mb={2}>Goals Progress</Typography>
              {goals.map(goal => (
                <Box key={goal._id} sx={{ mb: 2, p: 2, backgroundColor: 'grey.50', borderRadius: 2 }}>
                  ...
                  <Typography variant="caption" color="text.secondary">{goal.progress || 0}% complete</Typography>
                </Box>
              ))}
            </CardContent></Card>
          </Grid>
        ...
        <Box>
                ...
                <AccordionDetails sx={{ pt: 0 }}>
                  ...
                  {addTaskGoal === goal._id ? (
                    <Box sx={{ display: 'flex', gap: 1, mt: 1, pl: 4 }}>
                      <TextField size="small" placeholder="Task title..." value={newTaskTitle} onChange={e => setNewTaskTitle(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddTask(goal._id)} autoFocus sx={{ flex: 1 }} />
                      <Button size="small" onClick={() => handleAddTask(goal._id)} variant="contained">Add</Button>
                      <Button size="small" onClick={() => setAddTaskGoal(null)}>Cancel</Button>
                    </Box>
                  ) : (
                    <Button size="small" startIcon={<Add />} onClick={() => setAddTaskGoal(goal._id)} sx={{ mt: 1, ml: 3 }}>Add Task</Button>
                  )}
                </AccordionDetails>
              </Accordion>
            );
          ...
          {addGoalOpen ? (
            <Box sx={{ display: 'flex', gap: 1, mt: 2 }}>
              <TextField size="small" placeholder="Goal title..." value={newGoalTitle} onChange={e => setNewGoalTitle(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleAddGoal()} autoFocus sx={{ flex: 1 }} />
              <Button variant="contained" onClick={handleAddGoal}>Add Goal</Button>
              <Button onClick={() => setAddGoalOpen(false)}>Cancel</Button>
            </Box>
          ) : (
            <Button variant="outlined" startIcon={<Add />} sx={{ mt: 2 }} onClick={() => setAddGoalOpen(true)}>Add Goal</Button>
          )}
        </Box>
      )}
      ...
      <Dialog open={aiDialog === 'standup'} onClose={() => setAiDialog(null)} maxWidth="md" fullWidth>
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <AutoAwesome color="primary" /> Daily AI Standup Report
        </DialogTitle>
        <DialogContent>
          {aiLoading ? <LoadingSpinner message="Generating standup..." /> : standupReport && (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              {standupReport.todayPriorities?.length > 0 && <Box><Typography variant="subtitle2" fontWeight={700} mb={1}>Today's Priorities</Typography>{standupReport.todayPriorities.map((p, i) => <Typography key={i} variant="body2">• {p.task} ({p.assignee}) — {p.reason}</Typography>)}</Box>}
              {standupReport.blockers?.length > 0 && <Box><Typography variant="subtitle2" fontWeight={700} color="error.main" mb={1}>Blockers</Typography>{standupReport.blockers.map((b, i) => <Typography key={i} variant="body2">• {b.issue}: {b.suggestion}</Typography>)}</Box>}
              {standupReport.aiInsights?.length > 0 && <Box><Typography variant="subtitle2" fontWeight={700} mb={1}>AI Insights</Typography>{standupReport.aiInsights.map((ins, i) => <Typography key={i} variant="body2">💡 {ins}</Typography>)}</Box>}
            </Box>
          )}
        </DialogContent>
        <DialogActions><Button onClick={() => setAiDialog(null)}>Close</Button></DialogActions>
      </Dialog>

      <Dialog open={aiDialog === 'performance'} onClose={() => setAiDialog(null)} maxWidth="md" fullWidth>
        <DialogTitle sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          <AutoAwesome color="primary" /> Performance Analysis
        </DialogTitle>
        <DialogContent>
          {aiLoading ? <LoadingSpinner message="Analyzing performance..." /> : performanceReport && (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              <Box sx={{ display: 'flex', gap: 2 }}>
                ...
                <Box sx={{ textAlign: 'center', p: 2, backgroundColor: performanceReport.onTrack ? '#ECFDF5' : '#FEF2F2', borderRadius: 2, flex: 1 }}>
                  <Typography variant="h6" fontWeight={700} color={performanceReport.onTrack ? 'success.main' : 'error.main'}>{performanceReport.onTrack ? 'On Track' : 'At Risk'}</Typography>
                  <Typography variant="caption">Risk Level: {performanceReport.riskLevel}</Typography>
                </Box>
              </Box>
              {performanceReport.recommendations?.length > 0 && <Box><Typography variant="subtitle2" fontWeight={700} mb={1}>Recommendations</Typography>{performanceReport.recommendations.map((r, i) => <Typography key={i} variant="body2">• {r}</Typography>)}</Box>}
              {performanceReport.insights?.length > 0 && <Box><Typography variant="subtitle2" fontWeight={700} mb={1}>Insights</Typography>{performanceReport.insights.map((r, i) => <Typography key={i} variant="body2">💡 {r}</Typography>)}</Box>}
            </Box>
          )}
        </DialogContent>
        <DialogActions><Button onClick={() => setAiDialog(null)}>Close</Button></DialogActions>
