...
const PAGE_TITLES = {
  ...
  '/team': { title: 'Team', subtitle: 'Members and performance' },
  '/departments': { title: 'Departments', subtitle: 'Organizational structure' },
  '/ai': { title: 'AI Studio', subtitle: 'Intelligent project automation' },
};
...
export default function Header({ sidebarWidth = 268 }) {
  ...
  return (
    ...
    >
      <Toolbar sx={{ gap: 2, px: { xs: 2, sm: 3 }, minHeight: '60px !important' }}>
        {/* Page Title */}
        <Box sx={{ display: { xs: 'none', md: 'block' }, mr: 1 }}>
          <Typography variant="subtitle1" sx={{ fontWeight: 700, color: 'text.primary', lineHeight: 1.2, fontSize: '0.95rem' }}>
            {pageInfo.title}
          </Typography>
        ...
        <Box sx={{
          ...
          '&:focus-within': { borderColor: '#6366F1', backgroundColor: '#FEFEFF', boxShadow: '0 0 0 3px rgba(99,102,241,0.08)' }
        }}>
          <Search sx={{ color: '#94A3B8', fontSize: 16 }} />
          <InputBase placeholder="Search projects, tasks, people..." sx={{ fontSize: '0.82rem', flex: 1, color: 'text.primary', '& input::placeholder': { color: '#94A3B8' } }} />
        ...
        <Box
          ...
          sx={{ display: 'flex', alignItems: 'center', gap: 1, cursor: 'pointer', p: 0.5, pl: 1, borderRadius: 2.5, border: '1.5px solid #E2E8F0', backgroundColor: '#F8FAFC', '&:hover': { backgroundColor: '#F1F5F9', borderColor: '#CBD5E1' }, transition: 'all 0.15s' }}
        >
        ...
        <Menu
          ...
          PaperProps={{ sx: { mt: 0.75, minWidth: 200 } }}
        >
          <Box sx={{ px: 2, py: 1.5 }}>
            ...
            <Box sx={{ mt: 0.5 }}>
              <Chip label={user?.role?.name || 'Member'} size="small" sx={{ fontSize: '0.68rem', height: 18, backgroundColor: '#EEF2FF', color: '#4F46E5', fontWeight: 600 }} />
            </Box>
          </Box>
          <Divider sx={{ my: 0.5 }} />
          <MenuItem onClick={() => setAnchorEl(null)}>
            <Person sx={{ mr: 1.5, fontSize: 17, color: 'text.secondary' }} />
            <Typography variant="body2">My Profile</Typography>
          </MenuItem>
          <MenuItem onClick={() => setAnchorEl(null)}>
            <Settings sx={{ mr: 1.5, fontSize: 17, color: 'text.secondary' }} />
            <Typography variant="body2">Settings</Typography>
          </MenuItem>
          <Divider sx={{ my: 0.5 }} />
          <MenuItem onClick={() => { dispatch(logout()); setAnchorEl(null); }} sx={{ color: 'error.main', '&:hover': { backgroundColor: '#FEF2F2' } }}>
            <Logout sx={{ mr: 1.5, fontSize: 17 }} />
